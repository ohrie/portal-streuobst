        ...
        <div className="flex items-center justify-between mb-3 shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              aria-label={
                isCollapsed
                  ? "Messbereich ausklappen"
                  : "Messbereich einklappen"
              }
          ...
          <div id={panelContentId} className="flex flex-col min-h-0 flex-1">
            {features.length === 0 ? (
              <p className="text-sm text-gray-500 mb-3 shrink-0">
                Klicke auf eine Fläche zum Auswählen
              </p>
            ) : (
                    ...
                    <li key={`${f.sourceLayer}:${f.id}`} className="text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-700 truncate mr-2">
                          {f.name || `Fläche ${features.length - i}`}
                        </span>
                        <span className="text-nature font-medium whitespace-nowrap">
                          {formatArea(f.areaM2)}
                        </span>
                      </div>
                      {isLoading ? (
                        ...
                        <div className="text-xs text-green-700 mt-0.5">
                          🌳 {f.detectedTreeCount.toLocaleString("de-DE")}{" "}
                          erkannte Bäume
                        </div>
                ...
                {totalDetectedTrees != null && (
                  <div className="flex justify-between text-sm text-green-700">
                    ...
                    <span className="font-medium">
                      {totalDetectedTrees.toLocaleString("de-DE")}
                    </span>
                  </div>
                )}
                {totalTrees != null &&
                  ...
                  totalDetectedTrees == null && (
                    <div className="flex justify-between text-sm text-gray-600">
                      ...
                      <span className="font-medium">
                        {totalTrees.toLocaleString("de-DE")}
                      </span>
                    </div>
                  )}
              </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-medium text-gray-700">
              Bäume automatisch ermitteln
            </span>
            <span className="text-[10px] font-semibold bg-amber-100 text-amber-700 rounded px-1 py-0.5 leading-none">
              Beta
            </span>
            <div className="relative group/info">
              ...
              <div className="absolute bottom-full right-0 mb-2 w-60 bg-gray-900 text-white text-xs rounded-lg p-2.5 shadow-xl opacity-0 group-hover/info:opacity-100 transition-opacity pointer-events-none z-50 leading-relaxed">
                Automatische Ermittlung auf Basis des amtlichen DGM und DOM
                (Canopy Height Model, CHM). Die Berechnung ist fehlerbehaftet –
                insbesondere kleinere Bäume werden nicht zuverlässig erkannt.
                <br />
                <br />
                <span className="font-medium">
                  Derzeit nur in Baden-Württemberg + Bayern verfügbar.
                </span>
          ...
          <button
            ...
            aria-label={
              isTreeDetectEnabled
                ? "Automatische Baumerkennung deaktivieren"
                : "Automatische Baumerkennung aktivieren"
            }
            title={
              isTreeDetectEnabled
                ? "Baumerkennung deaktivieren"
                : "Baumerkennung aktivieren"
            }
          >
            <span
