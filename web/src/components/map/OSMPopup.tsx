...
function createIdEditorUrl(osmId: string): string | null {
  ...
  if (firstChar === "n") {
    ...
    const areaId = parseInt(numericId, 10);
    if (Number.isNaN(areaId)) return null;
...
function getDisplayOsmId(
  osmId: string,
): { displayId: string; type: string } | null {
  if (!osmId) return null;
...
export function createOSMPopupHTML(props: OSMPopupProps): string {
  ...
  const idEditorUrl = props.osmId
    ? createIdEditorUrl(String(props.osmId))
    : null;
  const displayOsmData = props.osmId
    ? getDisplayOsmId(String(props.osmId))
    : null;

  // Build properties HTML if showOSMTags is true
  let propsHtml = "";
  if (props.showOSMTags && props.properties) {
    const propsKeys = Object.keys(props.properties).filter(
      (k) => k !== "osm_id",
    );
    propsHtml = propsKeys.length
      ? `<ul class="text-sm space-y-1">${propsKeys
          .map(
            (k) =>
              `<li><strong>${k}:</strong> ${String(props.properties?.[k])}</li>`,
          )
          .join("")}</ul>`
