...
export default function OSMTagInfo({
  tags,
  wikiUrl,
  warning,
}: OSMTagInfoProps) {
  const showPopup = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    popup.className =
      "osm-info-popup fixed z-[9999] bg-white border-2 border-gray-300 rounded-lg p-3 shadow-2xl max-w-xs";

    const tagsHtml = tags
      .map(
        (tag) =>
          `<code class="bg-gray-100 px-1 rounded">${tag.key}=${tag.value}</code>`,
      )
      .join(" + ");
