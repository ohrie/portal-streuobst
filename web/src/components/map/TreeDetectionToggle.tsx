...
export default function TreeDetectionToggle({
  isActive,
  loadingCount,
  onToggle,
  isMobile,
}: TreeDetectionToggleProps) {
  ...
  return (
    <div className={widthClass}>
      <button
        type="button"
        onClick={() => {
          window.umami?.track("tree-detection-toggle", {
        ...
        aria-label={
          isActive ? "Baumerkennung beenden" : "Baumerkennung starten"
        }
      >
        {isLoading ? (
          <div
            className={`${iconClass} animate-spin rounded-full border-2 border-primary border-t-transparent`}
          />
        ) : (
          <svg
