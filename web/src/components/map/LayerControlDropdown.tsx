...
export default function LayerControlDropdown({
  ...
  const handleMainButtonClick = () => {
    window.umami?.track("layer-control-toggle", {
      label,
      action: isAnyActive ? "deactivate" : "activate",
    });
  ...
  return (
    <div ref={wrapperRef} className="relative group">
            ...
            {compactItems ? (
              ...
              layers.map((layer) => {
                const showGroupHeader =
                  layer.group !== undefined && layer.group !== lastGroup;
                ...
                return (
                  <div key={layer.id}>
                    ...
                    >
                      ...
                      <span
                        className={`text-xs font-medium flex-1 ${layer.isActive ? "text-primary" : "text-gray-700"}`}
                      >
                        {layer.label}
                      </span>
                      <span
                        className={`w-4 h-4 rounded border-2 shrink-0 flex items-center justify-center transition-colors ${layer.isActive ? "bg-primary border-primary" : "border-gray-300"}`}
                      >
                        {layer.isActive && (
                          <svg
      ...
      {isDisabled && disabledMessage && (
        <div
          className="absolute bottom-full left-1/2 -translate-x-1/2 pb-2 pointer-events-none z-50
      ...
      <div className="relative">
        <MapControlButton
          ...
          isMobile={isMobile}
        >
          <span className={isDisabled ? "opacity-40" : ""}>{icon}</span>
        </MapControlButton>
