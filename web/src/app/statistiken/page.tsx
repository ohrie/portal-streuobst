...
import { Calendar, Ruler, TreePine, TrendingUp } from "lucide-react";
...
export default function StatistikenPage() {
  ...
  const sorted = [...stats.laender].sort((a, b) =>
    activeChart === "count"
      ? b.wiesen_count - a.wiesen_count
      : b.wiesen_area_ha - a.wiesen_area_ha,
  );

  const top3 = [...stats.laender]
    .sort((a, b) => b.wiesen_area_ha - a.wiesen_area_ha)
    .slice(0, 3);
  const tableSorted = [...stats.laender].sort(
    (a, b) => b.wiesen_area_ha - a.wiesen_area_ha,
  );
  ...
  const chartData = sorted.map((l) => ({
    name: l.name
      .replace("Nordrhein-Westfalen", "NRW")
      .replace("Mecklenburg-Vorpommern", "M-V")
      .replace("Schleswig-Holstein", "S-H")
      .replace("Sachsen-Anhalt", "Sa-An")
      .replace("Rheinland-Pfalz", "RLP")
      .replace("Baden-Württemberg", "BW"),
    fullName: l.name,
  ...
  const _genusTotal = genusDist.reduce((s, g) => s + g.count, 0);
  ...
  return (
    <main>
      ...
      <section className="bg-background py-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-lg text-gray-600 max-w-2xl">
            Wie viele Obstwiesen sind in OpenStreetMap erfasst – und wie viel
            Fläche nehmen sie je Bundesland ein?
          </p>
          {stats.generated && (
      ...
      <section className="px-4 py-8">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl font-black text-secondary font-heading mb-5">
            Wiesen
          </h2>
        </div>
      ...
      <section className="px-4 py-8">
        <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
          ...
          <ResponsiveContainer
            width="100%"
            height={isMobile ? chartData.length * 36 + 20 : 420}
          >
            <BarChart
              data={chartData}
            ...
            >
              ...
              <Bar
                dataKey={
                  activeChart === "count" ? "wiesen_count" : "wiesen_area_ha"
                }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-500 uppercase text-xs tracking-wide">
                  ...
                  <th className="text-right px-6 py-3 font-semibold hidden md:table-cell">
                    Bäume
                  </th>
              ...
              <tbody className="divide-y divide-gray-50">
                {tableSorted.map((land, i) => {
                  const share =
                    totalHa > 0 ? (land.wiesen_area_ha / totalHa) * 100 : 0;
                  const isTop = i < 3;
                  return (
                    <tr
                      key={land.kuerzel}
        ...
        <section className="bg-background px-4 py-12 pb-16">
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-2xl font-black text-secondary font-heading">
                Bäume
              </h2>
            </div>
            <p className="text-gray-600 mb-6 max-w-2xl">
              Obstbäume aus OpenStreetMap (
              <code className="font-mono text-sm">natural=tree</code>), anhand
              der Gattung erkannt – bundesweit, innerhalb und außerhalb von
              Streuobstwiesen.
            </p>
            ...
            {genusChartData.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
                ...
                <p className="text-sm text-gray-500 mb-6">
                  Anzahl der erfassten Obstbäume je Gattung – inklusive der
                  Bäume innerhalb von Streuobstwiesen. Schreibvarianten sind
                  normalisiert zusammengefasst.
                </p>

                <ResponsiveContainer
                  width="100%"
                  height={genusChartData.length * 38 + 20}
                >
                  <BarChart
                    data={genusChartData}
                    layout="vertical"
                  ...
                  >
                    ...
                    />
                    <Tooltip
                      content={<CustomTooltip unit="Obstbäume" />}
